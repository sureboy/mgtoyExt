import {getDBFromName,postMessage} from './cache.js';
...
function computeDirection(dx, dy) {
    ...
    // 8方向扇区 (边界偏移22.5°)
    if ((angle >= 337.5 && angle <= 360) || (angle >= 0 && angle < 22.5)) {return 6; }  // 上
    if (angle >= 22.5 && angle < 67.5) {return 3;}    // 右上
    if (angle >= 67.5 && angle < 112.5) {return 2;}   // 右
    if (angle >= 112.5 && angle < 157.5) {return 1;}  // 右下
    if (angle >= 157.5 && angle < 202.5) {return 4;}  // 下
    if (angle >= 202.5 && angle < 247.5) {return 7;}  // 左下
    if (angle >= 247.5 && angle < 292.5) {return 8;}  // 左
    if (angle >= 292.5 && angle < 337.5) {return 9; } // 左上
    return 5;
}
// 更新UI方向码显示
function sendUDPControl(controlMsg){
    const db = getDBFromName(currentClient.name)||currentClient.rawDate;
    if (db){
        const senddb = {
            msg:new Uint8Array([db.Num,Number(controlMsg||0)|0xF0 ]),
            udp:{
...
function updateDirectionUI() {
    ...
    */
    ...
    sendUDPControl(currentDirection);
    return;
